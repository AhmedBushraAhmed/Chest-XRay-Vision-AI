...
MODEL_PATH = os.path.join(MODEL_DIR, "model_1_CNN.keras")

@st.cache_resource
def load_radiology_model():
    """Loads the trained Keras CNN model with automatic path fallback."""
    if not os.path.exists(MODEL_PATH):
        fallback = os.path.join(MODEL_DIR, "best_cnn_model.keras")
...
# ==============================================================================
def preprocess_radiograph(pil_img, target_size=(224, 224)):
    """Preprocesses input image into tensor matching model requirements."""
...
def generate_radiology_report(patient_info, primary_finding, probability, all_probs, original_img):
    ...
    pdf.cell(0, 8, "PULMOSCAN RADIOLOGY WORKSTATION", new_x="LMARGIN", new_y="NEXT", align='L')
    pdf.set_font("helvetica", '', 9)
    pdf.cell(0, 5, "Department of Diagnostic Imaging | Thoracic Consultation Summary", new_x="LMARGIN", new_y="NEXT", align='L')
    pdf.line(10, 24, 200, 24)
    
    pdf.ln(6)
    pdf.set_font("helvetica", 'B', 10)
    pdf.cell(40, 6, "Patient Identifier:", border=0)
    pdf.set_font("helvetica", '', 10)
    pdf.cell(50, 6, patient_info['id'], border=0)
    pdf.set_font("helvetica", 'B', 10)
    pdf.cell(30, 6, "Accession Date:", border=0)
    pdf.set_font("helvetica", '', 10)
    pdf.cell(50, 6, patient_info['date'], border=0, new_x="LMARGIN", new_y="NEXT")
    
    pdf.set_font("helvetica", 'B', 10)
    pdf.cell(40, 6, "Ordering Physician:", border=0)
    pdf.set_font("helvetica", '', 10)
    pdf.cell(50, 6, patient_info['physician'], border=0, new_x="LMARGIN", new_y="NEXT")
    
    pdf.line(10, 42, 200, 42)
    pdf.ln(6)
    pdf.set_font("helvetica", 'B', 12)
    pdf.cell(0, 7, "IMPRESSION & FINDINGS", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("helvetica", '', 11)
    pdf.cell(0, 6, f"Primary Classification: {primary_finding}", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, f"Confidence Likelihood: {probability:.2f}%", new_x="LMARGIN", new_y="NEXT")
    
    pdf.ln(4)
    pdf.set_font("helvetica", 'B', 10)
    pdf.cell(0, 6, "Differential Probability Distribution:", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("helvetica", '', 9)
    for name, prob in zip(CLASS_NAMES, all_probs):
        pdf.cell(0, 5, f" - {name}: {prob*100:.2f}%", new_x="LMARGIN", new_y="NEXT")
